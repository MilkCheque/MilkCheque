...
public class AuthenticationValidation {
  ...
  public void nameValidation(String name) {
    String message = null;
    if (name == null) message = "name is null";
    else if (name.isEmpty()) message = "name field is empty";
    else if (!name.matches("^[A-Za-z]+$")) message = "name format invalid";
  ...
  public void phoneNumberValidation(String phoneNumber) {
    ...
    try {
      if (phoneNumberExist(phoneNumber)) message = "phone number already exists";
    } catch (Exception e) {
  ...
  public void passwordValidation(String password) {
    String message = null;
    if (password == null) message = "password is null";
    if (password.isEmpty()) message = "password field is empty";
    if (!password.matches(passwordRegex)) message = "invalid password";
    if (message == null) return;
    throw new AuthenticationFormatException("-1", message);
  }
  ...
  public void emailValidation(String email) {
    ...
    if (email.isEmpty()) message = "email field is empty";
    if (!email.matches(emailRegex)) message = "invalid email";
    if (message == null) return;
    throw new AuthenticationFormatException("-1", message);
  }
  ...
  public boolean phoneNumberExist(String phoneNumber) {
    String message = null;
    Optional<StaffModel> staffModel = staffRepository.findByStaffPhone(phoneNumber);
    return staffModel.isPresent();
  }
  ...
  public void validateStaffSignup(SignUpRequest signUpRequest) {
    ...
    emailValidation(signUpRequest.getEmail());
    try {
      if (emailExist(signUpRequest.getEmail())) message = "email already exists";
    } catch (Exception e) {
      message = "emailExist: " + e.getMessage();
    }
    if (message != null) throw new AuthenticationFormatException("-1", message);
  }

  public void validateStaffLogin(LoginRequest loginRequest) throws Exception {
    ...
    if (!staffModel.isPresent()
        || !staffModel.get().getStaffPassword().equals(loginRequest.getPassword()))
